...
class JSONSchemaPlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [optparse.make_option('--json-schema-debug',
                                        dest='schema_debug',
                                        action="store_true",
        ...
        optp = optparser.add_option_group("JSON Schema-specific options")
        optp.add_options(optlist)
...
def find_path(module, path):
    ...
    logging.debug("Path now %s", pth)
    if len(pth) > 0:
        chs = [ch for ch in chs if ch.arg == pth[0]]
        pth = pth[1:]

def produce_module(module, path):
    logging.debug("in produce_module: %s %s with path %s", module.keyword, module.arg, path)
    res = {"title": module.arg, "$schema": "http://json-schema.org/schema#", "type": "object",
           "properties": {}}
    ...
    for child in module.i_children:
        if child.keyword in statements.data_definition_keywords:
            ...
            logging.debug("keyword not in data_definition_keywords: %s %s",
...
def produce_leaf(stmt):
    ...
    if rtype in NUMERIC_TYPE_TRANS_TBL:
        dtype = numeric_type_trans(rtype)[0]
        res = {arg: {"type": dtype}}
    elif rtype in OTHER_TYPE_TRANS_TBL:
        substmts = other_type_trans(rtype, stmt)
        logging.debug("in produce_leaf (from other_type_trans): %s %s %s",
                      stmt.keyword, stmt.arg, substmts)
        res = substmts
    elif rtype == "string":
        res = {arg: {"type": dtype}}
    else:
...
def numeric_type_trans(dtype):
    ttype = "string"
    tformat = None
    if dtype in NUMERIC_TYPE_TRANS_TBL:
        ttype = NUMERIC_TYPE_TRANS_TBL[dtype][0]
        tformat = NUMERIC_TYPE_TRANS_TBL[dtype][1]

    return ttype, tformat

def other_type_trans(dtype, stmt):
    return OTHER_TYPE_TRANS_TBL[dtype](stmt)
...
def qualify_name(stmt):
   ...
   # simple form of the member name MUST be used.
    ...
    #   stmt.keyword, stmt.arg, stmt.top.arg, stmt.parent.keyword, stmt.parent.arg,
    #   stmt.parent.top.arg)
    return stmt.arg
